...
(function() {
  var DefaultPkFactory, ObjectID, applyToRegistered, async, commit, commitWithGlobalLock, execute, rollback, _checkTransaction, _clone, _commitCore, _config, _db, _drainQueues, _enqueue, _enqueueOrCreateAndEnqueue, _executeImmediate, _hasAfter, _hasBefore, _immediateCounter, _immediately, _inSeries, _instructionsExistFor, _mongolize, _pending, _pendingImmediate, _pkFactory, _pushTransactionError, _queueCountersCollection, _queueLength, _queuePosition, _queues, _registry, _registryFind, _requeue, _retry, _revisionedUpdateChecks, _setUpTransaction, _shiftChain, _softwareVersion, _state, _transactionsCollection, _updateOneOptions, _updateTransactionState, _updateTransactionStatus, _withGlobalLock,
    __slice = [].slice;
  ...
  exports.db = {
    ...
    updateOneOpRollback: function(config, transaction, state, _arg, done) {
      ...
      return exports.db.updateOneOp(config, transaction, state, [collectionName, selector, rollbackOps, {}], function(err, result) {
        return done(err, true);
      });
    },
    revisionedUpdate: function(config, transaction, state, _arg, done) {
    ...
    revisionedUpdateRollback: function(config, transaction, state, _arg, done) {
      var collectionName, etc, newPartialDocument, oldPartialDocument, revisionName;
      collectionName = _arg[0], revisionName = _arg[1], newPartialDocument = _arg[2], oldPartialDocument = _arg[3], etc = 5 <= _arg.length ? __slice.call(_arg, 4) : [];
      if (etc.length !== 0) {
