...
(function() {
  var BuiltInObjects, DefaultPkFactory, MongoObjects, ObjectID, applyToRegistered, async, commit, commitWithGlobalLock, execute, rollback, t, _checkTransaction, _clone, _commitCore, _config, _db, _drainQueues, _enqueue, _enqueueOrCreateAndEnqueue, _executeImmediate, _hasAfter, _hasBefore, _immediateCounter, _immediately, _inSeries, _instructionsExistFor, _mongoFlatten, _mongoOpsFromTo, _mongolize, _pending, _pendingImmediate, _pkFactory, _processRevisions, _pushTransactionError, _queueCountersCollection, _queueLength, _queuePosition, _queues, _registry, _registryFind, _requeue, _setUpTransaction, _shiftChain, _softwareVersion, _state, _transactionsCollection, _updateManyOptions, _updateOneOptions, _updateOpInstruction, _updateOpRollback, _updateTransactionState, _updateTransactionStatus, _withGlobalLock,
    __slice = [].slice,
  ...
  commit = function(transactionOrFunction, done, retries, chainResults) {
    var doCommit;
    if (retries == null) {
      retries = 2;
    }
    doCommit = function(transaction, functionResults) {
      return _commitCore(transaction, function(err, status, transactionResults) {
        var result, _i, _j, _len, _len1;
        if (chainResults == null) {
          chainResults = [];
        }
        if (functionResults != null) {
          for (_i = 0, _len = functionResults.length; _i < _len; _i++) {
            result = functionResults[_i];
            chainResults.push(result);
          }
        }
        if (transactionResults != null) {
          for (_j = 0, _len1 = transactionResults.length; _j < _len1; _j++) {
            result = transactionResults[_j];
            chainResults.push(result);
          }
        }
        if (!(_hasBefore(transaction) || _hasAfter(transaction))) {
          return done.apply(null, [err, status].concat(__slice.call(chainResults)));
        } else {
          switch (status) {
            case 'Committed':
              if (_hasAfter(transaction)) {
                _shiftChain(transaction);
                return _requeue(transaction, function() {
                  var err, results, status, _k, _len2;
                  err = arguments[0], status = arguments[1], results = 3 <= arguments.length ? __slice.call(arguments, 2) : [];
                  if (results != null) {
                ...
                if (retries > 0) {
                  setTimeout(function() {
                    return _updateTransactionStatus(transaction, transaction.status, 'Queued', null, null, "" + status + " " + retries + " retries remaining", function(statusErr) {
                      ...
                      return commit(transaction, done, retries - 1, chainResults);
                    });
                  ...
                  return _updateTransactionStatus(transaction, transaction.status, 'ChainFailed', null, null, transaction.status, function(statusErr) {
                    if (statusErr != null) {
                      return done.apply(null, [statusErr, transaction.status].concat(__slice.call(chainResults)));
                    }
                    return done.apply(null, [err, 'ChainFailed'].concat(__slice.call(chainResults)));
                  });
                }
              }
              break;
  ...
  _commitCore = function(transaction, done) {
    ...
    return _queuePosition(transaction.queue, function(err, position) {
      ...
      return _transactionsCollection.update({
        ...
        return execute(transaction.instructions, transaction.execution.state, transaction, function(err, statuses, results) {
          if (err != null) {
