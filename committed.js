...
(function() {
  var BuiltInObjects, DefaultPkFactory, MongoObjects, ObjectID, applyToRegistered, async, commit, commitWithGlobalLock, execute, rollback, t, _checkInstructionRevisionConfig, _checkTransaction, _clone, _commitCore, _config, _db, _drainQueues, _enqueue, _enqueueOrCreateAndEnqueue, _executeImmediate, _hasAfter, _hasBefore, _immediateCounter, _immediately, _inSeries, _instructionsExistFor, _mongoFlatten, _mongoOpsFromTo, _mongolize, _pending, _pendingImmediate, _pkFactory, _processUpdateRevisions, _pushTransactionError, _queueCountersCollection, _queueLength, _queuePosition, _queues, _registry, _registryFind, _requeue, _retry, _setUpTransaction, _shiftChain, _softwareVersion, _state, _transactionsCollection, _updateOneOptions, _updateTransactionState, _updateTransactionStatus, _withGlobalLock,
    __slice = [].slice,
  ...
  MongoObjects = (function() {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      t = _ref[_i];
      _results.push(require('mongodb')[t]);
    }
    return _results;
  })();
  ...
  _checkInstructionRevisionConfig = function(instructionName, config, collectionName, revisionNames) {
    var revisionName, _i, _len;
  ...
  _processUpdateRevisions = function(config, collectionName, revisions) {
    var key, revisionNames, revisionNumbers;
    if (revisions == null) {
  ...
  exports.db = {
    ...
    insert: function(config, transaction, state, _arg, done) {
      ...
      _ref = _processUpdateRevisions(config, collectionName, revisions), revisionNames = _ref[0], revisionNumbers = _ref[1];
      ...
      if (revisions.length > 0 && documents.some(function(d) {
      ...
      return config.db.collection(collectionName, {
        ...
        for (_i = 0, _len = documents.length; _i < _len; _i++) {
          ...
          if (revisions.length > 0 && (d.revision == null)) {
            d.revision = {};
            for (_j = 0, _len1 = revisions.length; _j < _len1; _j++) {
              revisionName = revisions[_j];
    ...
    updateOneOp: function(config, transaction, state, _arg, done) {
      var collectionName, error, etc, revisionNames, revisionNumbers, revisions, rollbackProjector, selector, updateOps, _ref;
      ...
      if ((updateOps.__inc != null) && revisionNames.some(function(name) {
        return updateOps.__inc["revision." + name];
      })) {
        ...
        return collection.find(mongoSelector, mongoProjector).toArray(function(err, docs) {
          ...
          return _updateTransactionState(transaction, function(err) {
            ...
            executeUpdate = function() {
              var mongoUpdateOps, revisionName, _i, _len;
              ...
              return collection.update(mongoSelector, mongoUpdateOps, _updateOneOptions, function(err, updated) {
                if (err != null) {
                  return done(err, null);
                }
                if (updated !== 1) {
                  transaction.execution.info.push("updateOneOp can update only one document, using " + (JSON.stringify(mongoSelector)) + " " + updated + " were updated");
                  return done(null, false);
                }
                return done(null, true);
              });
            ...
            if (revisionNumbers != null) {
              ...
              if (!matches.every(function(x) {
                ...
                transaction.execution.info.push("updateOneOp failed to find documents to match the required revisions. \nFound: " + (JSON.stringify(state.updated)) + ". \nRequired: " + (JSON.stringify(revisionNumbers)) + ".");
                return done(null, false);
              } else {
    ...
    updateOneOpRollback: function(config, transaction, state, _arg, done) {
      var collectionName, etc, revisionNames, revisionNumbers, revisions, rollbackProjector, selector, updateOps, _ref;
      collectionName = _arg[0], selector = _arg[1], updateOps = _arg[2], rollbackProjector = _arg[3], revisions = _arg[4], etc = 6 <= _arg.length ? __slice.call(_arg, 5) : [];
      if (etc.length !== 0) {
        return done(new Error("too many values passed to updateOneOp"));
      }
      ...
      return config.db.collection(collectionName, {
        ...
        if (state.updated == null) {
          ...
          revisionSelector = {
            _id: state.updated._id
          };
          _ref1 = state.updated.revision;
          ...
          return collection.count(revisionSelector, function(err, count) {
            var flattened, from, key, mongoRollbackOps, mongoSelector, op, originalDotPaths, path, renamedTo, settyOperators, to, unsetDotPaths, updatedDotPaths, value, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _ref2;
            if (count === 1) {
              return done(null, true);
            } else if (count === 0) {
              mongoRollbackOps = {
              ...
              if (updateOps['__rename'] != null) {
                renamedTo = (function() {
                  var _ref3, _results;
                  _ref3 = updateOps['__rename'];
                  _results = [];
                  for (from in _ref3) {
                    to = _ref3[from];
                    _results.push(to);
              ...
              return collection.update(mongoSelector, mongoRollbackOps, _updateOneOptions, function(err, updated) {
                if (err != null) {
                  return done(err, false);
                }
                if (updated !== 1) {
    ...
    updateOneDoc: function(config, transaction, state, _arg, done) {
      var collectionName, error, etc, newPartialDocument, oldPartialDocument, revisionName, revisionNames;
      ...
      return config.db.collection(collectionName, {
        ...
        var key, oldSelector, value, _i, _len, _ref, _ref1;
        if (err != null) {
          return done(err, false);
        }
        oldSelector = {};
        _ref = _mongoFlatten(oldPartialDocument);
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        return collection.count(oldSelector, function(err, count) {
          ...
          return _updateTransactionState(transaction, function(err) {
            ...
            }, mongoOps, _updateOneOptions, function(err, updated) {
              if (err != null) {
                return done(err, null);
              }
              if (updated !== 1) {
