...
(function() {
  var DefaultPkFactory, ObjectID, applyToRegistered, async, commit, execute, rollback, _clone, _db, _drainQueues, _enqueueOrCreateAndEnqueue, _instructionsExistFor, _mongolize, _pkFactory, _pushTransactionError, _queueCountersCollection, _queuePosition, _queues, _quietly, _registry, _registryFind, _revisionedUpdateChecks, _softwareVersion, _state, _transactionsCollection, _updateOneOptions, _updateTransactionStatus,
    __slice = [].slice;
  ...
  rollback = function(transaction, failStatus, done) {
    var rollbackInstructions, t, _i, _len;
    ...
    if (transaction.rollback != null) {
      ...
      for (_i = 0, _len = rollbackInstructions.length; _i < _len; _i++) {
        t = rollbackInstructions[_i];
        t.name = t.name + 'Rollback';
      }
    }
    return execute(rollbackInstructions, transaction.data, function(rollbackErr, result) {
